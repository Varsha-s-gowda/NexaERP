...
const printChallan = (challan: SalesChallan) => {
  const printWindow = window.open('', '_blank');
  if (!printWindow) return;

  const itemsHtml = challan.items.map(item => `
    <tr>
      <td style="border: 1px solid #ddd; padding: 8px;">${item.productName}</td>
      <td style="border: 1px solid #ddd; padding: 8px;">${item.productCode}</td>
      <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${item.quantity}</td>
      <td style="border: 1px solid #ddd; padding: 8px; text-align: right;">₹${item.sellingPrice.toFixed(2)}</td>
  ...
  const html = `
    <html>
      ...
      <body>
        <div class="header">
          <div>
            <div class="title">NexaERP</div>
            <div>Enterprise Management System</div>
          </div>
          <div style="text-align: right;">
            <div style="font-size: 18px; font-weight: bold;">SALES CHALLAN</div>
            <div>No: ${challan.challanNumber}</div>
            <div>Date: ${new Date(challan.createdAt).toLocaleDateString()}</div>
          </div>
        </div>

        <hr style="border: none; border-top: 2px solid #1a1a2e; margin: 20px 0;" />

        <div class="details" style="display: flex; justify-content: space-between;">
          <div>
            <strong>Customer Details:</strong><br />
            Name: ${challan.customerName}<br />
            Business: ${challan.businessName || '-'}<br />
          </div>
          <div style="text-align: right;">
            <strong>Challan Information:</strong><br />
            Status: ${challan.status}<br />
            Payment Status: ${challan.paymentStatus || 'PENDING'}<br />
          </div>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 20px;">
          <thead>
            <tr style="background-color: #f2f2f2;">
              ...
              <th style="border: 1px solid #ddd; padding: 8px; text-align: left;">SKU/Code</th>
        ...
        <div style="margin-top: 20px; text-align: right; line-height: 1.8;">
          ...
          <div style="font-size: 18px; color: #1a1a2e;"><strong>Total Amount:</strong> ₹${challan.totalAmount.toFixed(2)}</div>
          <div><strong>Amount Paid:</strong> ₹${(challan.amountPaid || 0).toFixed(2)}</div>
          <div><strong>Outstanding Amount:</strong> ₹${(challan.totalAmount - (challan.amountPaid || 0)).toFixed(2)}</div>
        </div>

        <div class="footer" style="display: flex; justify-content: space-between; font-size: 12px; color: #666;">
          <div>Generated by NexaERP</div>
...
export default function SalesChallans() {
  ...
  const canRecordPayment = isAdmin || isAccounts;
  ...
  const recordPaymentMutation = useMutation({
    mutationFn: async ({ id, amount }: { id: string; amount: number }) => {
      await api.post(`/sales-challans/${id}/payment`, { amount });
    },
    onSuccess: () => {
                      ...
                      {challans.map((challan) => {
                        return (
                          ...
                          >
                            ...
                            {/* Total */}
                            <td className="px-4 py-2 whitespace-nowrap text-sm font-semibold text-gray-900 text-right">
                              {formatCurrency(challan.totalAmount)}
                            </td>

                            {/* Paid Amount */}
                            <td className="px-4 py-2 whitespace-nowrap text-sm text-green-600 text-right">
                              {formatCurrency(challan.amountPaid || 0)}
                            </td>

                            {/* Outstanding */}
                            <td className="px-4 py-2 whitespace-nowrap text-sm text-red-600 text-right">
                              {formatCurrency(challan.outstandingAmount ?? (challan.totalAmount - (challan.amountPaid || 0)))}
                            </td>

                            {/* Payment Status badge */}
                            <td className="px-4 py-2 whitespace-nowrap text-center">
                              <span
                                className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full ${getPaymentStatusColor(challan.paymentStatus)}`}
                              >
                            ...
                            <td className="px-4 py-2 whitespace-nowrap text-center">
                              <span
                                className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-medium rounded-full ${getChallanStatusColor(challan.status)}`}
                              >
                  ...
                  <div className="flex justify-end">
                    <div className="w-64 space-y-2">
                      ...
                      <div className="flex justify-between text-sm text-green-600">
                        ...
                        <span>{formatCurrency(selectedChallan.amountPaid || 0)}</span>
                      </div>
                      <div className="flex justify-between text-sm text-red-600">
                        <span>Outstanding</span>
                        <span>{formatCurrency(selectedChallan.outstandingAmount ?? (selectedChallan.totalAmount - (selectedChallan.amountPaid || 0)))}</span>
                      </div>
                      <div className="flex justify-between text-sm text-gray-600">
                        <span>Payment Status</span>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${getPaymentStatusColor(selectedChallan.paymentStatus)}`}>
                          {selectedChallan.paymentStatus || 'PENDING'}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      {showPaymentModal && selectedChallan && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full shadow-xl">
            ...
            <div className="p-4">
              <div className="bg-slate-50 rounded-lg p-3 text-sm text-gray-700 mb-4">
                <p><span className="font-semibold">Challan:</span> {selectedChallan.challanNumber}</p>
                <p><span className="font-semibold">Total Amount:</span> {formatCurrency(selectedChallan.totalAmount)}</p>
                <p><span className="font-semibold">Already Paid:</span> {formatCurrency(selectedChallan.amountPaid || 0)}</p>
                <p><span className="font-semibold text-red-600">Outstanding:</span> {formatCurrency(selectedChallan.outstandingAmount ?? (selectedChallan.totalAmount - (selectedChallan.amountPaid || 0)))}</p>
              </div>
              ...
              <form
                onSubmit={(e) => {
                  ...
                  const outstanding = selectedChallan.outstandingAmount ?? (selectedChallan.totalAmount - (selectedChallan.amountPaid || 0));
                  if (isNaN(amt) || amt <= 0) {
                    setPaymentError('Please enter a valid amount greater than 0');
                    return;
                  }
                  if (amt > outstanding) {
                    setPaymentError(`Amount exceeds outstanding amount (${formatCurrency(outstanding)})`);
                    return;
                  }
                  recordPaymentMutation.mutate({ id: selectedChallan.id, amount: amt });
                }}
              >
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                <div className="flex items-center justify-end gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2 bg-[#1a1a2e] text-white rounded-lg transition-colors text-sm disabled:opacity-60 disabled:cursor-not-allowed"
                  >
